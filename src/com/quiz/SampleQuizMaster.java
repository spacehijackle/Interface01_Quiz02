...
public class SampleQuizMaster implements QuizMaster
{
	...
	public Quiz poll()
	{
		...
		//quiz = new NoCorrectAnswerQuiz(quiz);  // 絶対正解させないクイズのラッパーを介す
		quiz = new QuizCounter(quiz);          // 正解数等をカウントするラッパーを介す
		return quiz;
	}
	...
	{
		return ((float)_correctCount / (float)_questionCount);
	}

	/**
	 * クイズの出題数、正解数をカウントするクラス
	 */
	class QuizCounter implements Quiz
	{
		// 対象クイズ
		Quiz target;
		...
		{
			boolean isCorrect = target.isCorrect(reply);
			if(isCorrect)
			{
				// 正解の場合、正解数をインクリメント
				_correctCount++;
